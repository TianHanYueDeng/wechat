...
func (srv *Server) handleRequest() (replies []*message.Reply, err error) {
	//set isSafeMode
	...
	replies = srv.messageHandler(mixMessage)
	return
}
...
func (srv *Server) buildResponse(replies []*message.Reply) (err error) {
	defer func() {
		if e := recover(); e != nil {
...
func (srv *Server) Send() (err error) {
	...
	if srv.isSafeMode {
		//安全模式下对消息进行加密
		for i, reply := range srv.responseRawXMLMsg {
			var encryptedMsg []byte
			encryptedMsg, err = util.EncryptMsg(srv.random, reply, srv.AppID, srv.EncodingAESKey)
			if err != nil {
				return
			}
			...
			replyMsg[i] = message.ResponseEncryptedXMLMsg{
	...
	if replyMsg != nil && len(replyMsg) > 0 {
		for _, reply := range replyMsg {
			srv.XML(reply)
		}
	}
	return
